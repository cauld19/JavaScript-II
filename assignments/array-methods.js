...
// let firstAndLast = runners.filter ((name) => {
//     fullName.push(`${name.first_name}  ${name.last_name}`);
// });

// let firstAndLast = runners.filter ((name) => fullName.push(`${name.first_name}  ${name.last_name}`));

// let firstAndLast = runners.forEach (function(name) {
//     fullName.push(`${name.first_name}  ${name.last_name}`);
// });

let firstAndLast = runners.forEach ((name) => fullName.push(`${name.first_name}  ${name.last_name}`));
...
// let upperCase = runners.map(function(name) {
//     return name.first_name.toUpperCase();
// });

let upperCase = runners.map ((name) => name.first_name.toUpperCase()); allCaps.push(upperCase);
...
// let donationReducer = runners.reduce (function(total, donations) {
//     return total + donations.donation;
// },0);

let donationReducer = runners.reduce ((total, donations) => total + donations.donation, 0);

ticketPriceTotal.push(donationReducer);
...
// Problem 1
//calculate the amount of large shirt size you need to order
let shirtSizes = [];
let sizeFinder = runners
    .filter((shirt) => (shirt.shirt_size === "L"))
    shirtSizes.push(sizeFinder.length);
...
let avDonation = runners.reduce ((a,b) => a + b.donation / 50, 0);
console.log(avDonation);
